...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(
      "devmind.chat",
      viewProvider.getChatView(),
    ),
    vscode.window.registerWebviewViewProvider(
      "devmind.context",
      viewProvider.getContextView(),
    ),
    vscode.window.registerWebviewViewProvider(
      "devmind.history",
      viewProvider.getHistoryView(),
    ),
  );
      ...
      async (request, chatContext, stream, token) => {
        ...
        if (request.prompt.startsWith("/gitmate")) {
          ...
          const result = await devMindManager.runAgent(
            ...
            {
              git: {
                diff: request.prompt.substring("/gitmate ".length).trim(),
                status: "",
                branch: "",
                log: "",
              },
          ...
          stream.markdown(
            "Hello! How can I help you today? Try @devmind /gitmate to generate a commit message.",
          );
        }
  ...
  vscode.window.showInformationMessage(
    "🧠 DevMind is ready! Your AI pair programmer is standing by.",
  );
}
