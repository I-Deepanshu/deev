...
export class DevFlowAgent {
    ...
    public getCapabilities(): AgentCapability[] {
        return [
            ...
            },
            {
                ...
                description: 'Suggests improvements or new CI/CD pipeline configurations.',
                supportedLanguages: ['*'],
                supportedFileTypes: ['*'],
                requiresContext: ['problemStatement', 'filePath', 'surroundingCode'],
                outputTypes: ['message']
    ...
    public async automateWorkflow(context: ContextData, cancellationToken?: vscode.CancellationToken): Promise<AgentResult> {
        try {
            ...
            return {
                success: false,
                error: error.message || 'An unexpected error occurred during workflow automation',
                agentType: 'devflow',
                executionTime: 0,
            };
        }
    ...
    public async generateScript(context: ContextData, cancellationToken?: vscode.CancellationToken): Promise<AgentResult> {
        try {
            ...
            return {
                success: false,
                error: error.message || 'An unexpected error occurred during script generation',
                agentType: 'devflow',
                executionTime: 0,
            };
        }
