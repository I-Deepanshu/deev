...
export class AgentOrchestrator {
   ...
   */
  suggestAgent(context: ContextData): {
    agentType: AgentType;
    confidence: number;
    reasoning: string;
  } {
  ...
  getAllAgentSuggestions(
    ...
    if (
      (context.architecturalPatterns?.length || 0) === 0 &&
      (context.projectStructure?.files.length || 0) > 10
    ) {
      confidence += 0.3;
  ...
  private initializeAgents(): void {
    this.agents.set(
      "architect",
      new ArchitectAgent(this.llmProvider, this.contextAnalyzer),
    );
    this.agents.set(
      "codesmith",
      new CodeSmithAgent(this.llmProvider, this.contextAnalyzer),
    );
    this.agents.set(
      "bughunter",
      new BugHunterAgent(this.llmProvider, this.contextAnalyzer),
    );
