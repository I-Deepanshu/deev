...
import { IAgent, AgentType, AgentResult, AgentCapability, AgentStatus, AnalysisResult, Alternative, DocumentationOutput } from './types';

export class ArchitectAgent implements IAgent {
    readonly type: AgentType = 'architect';
    readonly name: string = 'Architect';
    readonly description: string = 'Proposes scalable designs and architecture changes';
        ...
        try {
            ...
            switch (analysisType) {
                case 'project_structure':
                    result = await this.analyzeProjectStructure(context, cancellationToken);
                    break;
                ...
                case 'technology_stack':
                    result = await this.analyzeTechnologyStack(context, cancellationToken);
                    break;
                case 'architecture_review':
                    result = await this.performArchitectureReview(context, cancellationToken);
                    break;
                default:
                    result = await this.performGeneralArchitecturalAnalysis(context, cancellationToken);
            }
        ...
        return [
            {
                ...
                description: 'Analyzes and suggests improvements to project structure and organization',
                supportedLanguages: ['typescript', 'javascript', 'python', 'java', 'csharp', 'go'],
                supportedFileTypes: ['.ts', '.js', '.py', '.java', '.cs', '.go', '.json', '.yaml', '.md'],
            ...
            },
            {
                ...
                description: 'Evaluates and suggests improvements for system scalability',
                supportedLanguages: ['typescript', 'javascript', 'python', 'java', 'go'],
                supportedFileTypes: ['.ts', '.js', '.py', '.java', '.go'],
                requiresContext: ['projectStructure', 'dependencies', 'architecturalPatterns'],
            ...
            },
            {
                ...
                supportedLanguages: ['*'],
                supportedFileTypes: ['package.json', 'requirements.txt', 'pom.xml', 'go.mod'],
    ...
    validateContext(context: ContextData): { valid: boolean; reason?: string } {
        ...
        if (!context.projectStructure || context.projectStructure.files.length === 0) {
            return { valid: false, reason: 'No project structure available for analysis' };
        }
    ...
    private determineAnalysisType(context: ContextData): string {
        ...
        if ((context.complexity || 0) > 15 || (context.projectStructure?.files.length || 0) > 50) {
            return 'scalability';
        }
        
        if (context.dependencies?.production && Object.keys(context.dependencies.production).length > 10) {
            return 'technology_stack';
        }
        
        if (context.architecturalPatterns?.length && context.projectStructure?.files.length) {
    ...
    private async performGeneralArchitecturalAnalysis(context: ContextData, cancellationToken?: vscode.CancellationToken): Promise<AgentResult> {
        ...
        return this.parseArchitecturalResponse(llmResponse.content!, 'general', context);
    }
...
Provide practical, context-aware architectural advice.`;
    ...
    private parseArchitecturalResponse(content: string, analysisType: string, context: ContextData): AgentResult {
        ...
        for (const line of lines) {
            ...
            // Detect sections
            if (trimmedLine.toLowerCase().includes('issue:') || trimmedLine.toLowerCase().includes('problem:')) {
                ...
                suggestions.push(trimmedLine.replace(/^(recommendation:|suggestion:)/i, '').trim());
            ...
            } else if (currentAnalysis.title && !currentAnalysis.description && trimmedLine) {
        ...
        if (analysisType === 'architecture_review' || analysisType === 'project_structure') {
            documentation.push({
                type: 'readme',
    ...
    private formatDependencies(dependencies: any): string {
        const prod = Object.entries(dependencies.production || {}).map(([name, version]) => `  ${name}: ${version}`);
        const dev = Object.entries(dependencies.development || {}).map(([name, version]) => `  ${name}: ${version}`);
    ...
    private determineSeverity(text: string): 'info' | 'warning' | 'error' | 'critical' {
        ...
        if (lowerText.includes('error') || lowerText.includes('major')) return 'error';
        if (lowerText.includes('warning') || lowerText.includes('minor')) return 'warning';
        return 'info';
    }
    
    private calculateConfidence(analysisResults: AnalysisResult[], suggestions: string[]): number {
    ...
    private generateArchitectureDoc(content: string, context: ContextData): string {
        return `# Architecture Analysis\n\n## Project Overview\n\n- **Root:** ${context.projectRoot}\n- **Files:** ${context.projectStructure?.files.length || 0}\n- **Patterns:** ${context.architecturalPatterns?.join(', ') || 'N/A'}\n\n## Analysis Results\n\n${content}\n\n## Generated by DevMind Architect Agent\n\n*Last updated: ${new Date().toISOString()}*`;
    }
    
    private generateNextSteps(analysisType: string, analysisResults: AnalysisResult[]): string[] {
        const steps: string[] = [];
