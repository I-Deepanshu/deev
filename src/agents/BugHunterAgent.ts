...
export class BugHunterAgent implements IAgent {
    ...
    try {
      ...
      switch (debugType) {
        ...
        case "performance_issue":
          result = await this.analyzePerformanceIssue(
            context,
            cancellationToken,
          );
          break;
        ...
        case "integration_issue":
          result = await this.analyzeIntegrationIssue(
            context,
            cancellationToken,
          );
          break;
        case "security_vulnerability":
          result = await this.analyzeSecurityVulnerability(
            context,
            cancellationToken,
          );
          break;
        default:
          result = await this.performGeneralDebugging(
            context,
            cancellationToken,
          );
      }
    ...
    return [
      {
        ...
        description:
          "Analyzes runtime errors and exceptions with stack trace analysis",
        supportedLanguages: [
          "typescript",
          "javascript",
          "python",
      ...
      },
      {
        ...
        description:
          "Identifies potential memory leaks and resource management issues",
        supportedLanguages: [
          "typescript",
          "javascript",
          "python",
  ...
  validateContext(context: ContextData): { valid: boolean; reason?: string } {
    ...
    if (
      ...
      context.surroundingCode.trim().length === 0
    ) {
  ...
  private determineDebugType(context: ContextData): string {
    ...
    if (
      errorMessages.some(
        (msg) => msg.includes("Error:") || msg.includes("Exception:"),
      )
    ) {
    ...
    if (
      errorMessages.some(
        (msg) => msg.includes("Type") || msg.includes("type"),
      ) ||
    ...
    // Performance issues
    if (
      (context.complexity && context.complexity > 15) ||
  ...
  private async analyzeRuntimeError(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
    ...
    return this.parseDebuggingResponse(
      llmResponse.content!,
      "runtime_error",
      context,
    );
  ...
  private async analyzeMemoryLeak(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
  ...
  private async analyzeAsyncIssue(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
  ...
  private async analyzeTypeError(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
  ...
  private async analyzeIntegrationIssue(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
  ...
  private async analyzeSecurityVulnerability(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
  ...
  private async performGeneralDebugging(
    context: ContextData,
    cancellationToken?: vscode.CancellationToken,
    stream?: vscode.ChatResponseStream,
  ): Promise<AgentResult> {
...
Provide comprehensive debugging guidance and improvement suggestions.`;
  ...
  private parseDebuggingResponse(
    ...
    const confidence = this.calculateDebuggingConfidence(
      analysisResults,
      debugType,
    );
  ...
  private generateDebuggingNextSteps(
    debugType: string,
    analysisResults: AnalysisResult[],
  ): string[] {
    const steps: string[] = [];
