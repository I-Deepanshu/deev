...
export class AuditTrail {
     ...
     */
    async logAgentAction(agentType: AgentType, context: ContextData, result: any): Promise<void> {
        if (!this.enabled) return;
    ...
    private summarizeContext(context: ContextData): any {
        return {
            ...
            hasErrorMessages: Array.isArray(context.errorMessages) && context.errorMessages.length > 0
        };
    }
    ...
    private summarizeResult(result: any): any {
        return {
            ...
            hasAnalysisResults: result.analysisResults && result.analysisResults.length > 0
        };
    }
