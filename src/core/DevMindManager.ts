...
export class DevMindManager {
    ...
    public async handleChatMessage(message: string, agentType: AgentType = 'default'): Promise<void> {
        this.postMessageToWebview('addMessage', `DevMind: Processing your request...`);

        if (agentType && agentType !== 'default') {
            this.postMessageToWebview('addMessage', `DevMind: Activating ${agentType} agent with message: "${message}"`);
            try {
                await this.runAgent(agentType);
                this.postMessageToWebview('addMessage', `DevMind: ${agentType} agent activated successfully.`);
            } catch (error) {
                this.postMessageToWebview('addMessage', `DevMind: Failed to activate ${agentType} agent. Error: ${error instanceof Error ? error.message : String(error)}`);
            }
        } else {
            this.postMessageToWebview('addMessage', `DevMind: Received message: "${message}".`);
    ...
    async activateContextualAgent(): Promise<void> {
        try {
            ...
            const selection = await vscode.window.showQuickPick([
                { label: `🧠 ${suggestedAgent.toUpperCase()} (Recommended)`, value: suggestedAgent },
    ...
    async runAgent(agentType: AgentType, context?: ContextData, stream?: vscode.ChatResponseStream): Promise<void> {
        try {
            ...
            if (!this.privacyManager.canProcessContext(currentContext)) {
                vscode.window.showWarningMessage('Privacy mode prevents processing this context.');
                return;
            }
    ...
    async onConfigurationChange(): Promise<void> {
        ...
        const privacyMode = config.get('privacyMode', true) ? PrivacyMode.ENHANCED : PrivacyMode.STANDARD;
    ...
    private async getRefactorIntent(): Promise<string> {
        const intent = await vscode.window.showInputBox({
            prompt: 'What would you like to refactor? (e.g., "extract method", "improve performance", "add error handling")',
