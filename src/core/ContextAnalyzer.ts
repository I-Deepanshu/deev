...
export class ContextAnalyzer {
   ...
   */
  private async analyzeImmediateContext(
    document: vscode.TextDocument,
  ...
  private async analyzeProjectContext(): Promise<Partial<ContextData>> {
    ...
    const architecturalPatterns =
      this.detectArchitecturalPatterns(projectStructure);
   ...
   */
  private async analyzeExternalContext(
    dependencies: Dependencies,
  ): Promise<Partial<ContextData>> {
  ...
  private getCurrentFunction(
    ...
    for (let i = position.line; i >= 0; i--) {
      ...
      const functionMatch = line.match(
        /(?:function|def|async\s+function|const\s+\w+\s*=\s*(?:async\s+)?\(|class\s+\w+|interface\s+\w+)\s+(\w+)/i,
      );
      if (functionMatch) {
  ...
  private async getProjectStructure(): Promise<ProjectStructure> {
    ...
    try {
      ...
      const testFiles = fileInfos
        .filter((f) => this.isTestFile(f.path))
        .map((f) => f.path);
      const configFiles = fileInfos
        .filter((f) => this.isConfigurationFile(f.path))
        .map((f) => f.path);
  ...
  private checkMissingDocumentation(text: string): boolean {
    ...
    return (
      ...
      commentMatches.length < functionMatches.length * 0.5
    );
  }
  ...
  private async getLibraryVersions(
    dependencies: Dependencies,
  ): Promise<LibraryInfo[]> {
